...
def handle_connection(data):
    ...
    if data:
        ...
        cur.close()
        conn.close()
        if current_user.strip() in connected_users:
            connected_users[current_user.strip()].append(request.sid)
        else :
            connected_users[current_user.strip()]=[request.sid]
        for connection in connected_users[current_user.strip()]:
            socketio.emit('value_updated', {'money':float(usermoney[0]) ,'user':current_user},room=connection) 
        print("New user sign in!\nThe users are: ",current_user)
    else:
        connected_users[current_user.strip()].remove(request.sid)
        if len(connected_users[current_user.strip()])==0:
            del connected_users[current_user.strip()]

        print("user logout !\nThe users are: ",current_user.strip(),"sid: ",request.sid)
...
def buyBoosters():
    ...
    if(amount<usermoney):
        ...
        for connection in connected_users[current_user.strip()]: 
            ...
            socketio.emit('value_updated', {'money': usermoney - amount, 'user': current_user}, room=connection)
...
def getUserData():
    ...
    conn.close()
    # for connection in connected_users[current_user.strip()]:
    #     socketio.emit('value_updated', {'money': float(results[5]), 'user': current_user}, room=connection)
